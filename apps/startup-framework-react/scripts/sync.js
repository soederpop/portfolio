...
async function main() {
  ...
  } else if (runtime.argv.summarize) {
    await summarize({ siteCopy, content, worksheet })
  } else {
...
async function summarize({ siteCopy, content }) {
  await siteCopy.loadAll()
  const items = siteCopy.data.siteCopy
  const { uniq } = runtime.lodash

  const phrasesData = uniq(items.map(v => v.value))

  const worksheet = await siteCopy.ws('phrases')

  for(let phrase of phrasesData)  {
    await worksheet.addRow({ originalContent: phrase, actualContent: phrase })
  }
}
...
async function updateRemote({ siteCopy, worksheet }) {
  ...
  console.log(`Searched ${components.length} components.`)
  console.log(`Found a total ${records.length} of string literal type nodes`)
...
function processAstPath(path, options = {}) {
  ...
  return {
    ...
    value: path.node.value && String(path.node.value).trim(),
    path() {
      return path
    },
  }
...
async function processFile(script, options = {}) {
  ...
  console.log(`Processing File: ${script.name}`)
  ...
  const results = [...jsxAttributes, ...jsxText].map(result => {
    return {
      ...
      value: sanitize(String(result.value).trim()),
    }
  })
...
async function loadComponentFiles() {
  const components = await runtime.contentManager.sourceModules.filter(({ file }) =>
    file.relative.match(/^src.(blocks)/)
  )
...
async function loadSiteCopy() {
  ...
  console.log('Loading Site Copy from Google Sheet')
...
function sanitize(input) {
  if (input && input.replace(/\s/g,'').match(/^\+\d+$/)) {
